...
# New clean directory for plots
OUTPUT_PREFIX="ANALYSIS/results_visualization/Evolution"
...
# Added options(warn=1) to see warnings immediately
singularity exec --bind $PWD:/data --pwd /data "$IMG_PATH" \
    Rscript -e "options(warn=1); source('plot_evolution_kitchen_sink.R')" \
    "$RESULTS_DIR" \
    "$OUTPUT_PREFIX" \
    "$METADATA_FILE" \
    "$CONTRASTS_FILE" \
...
# Updated MultiQC to point to ANALYSIS/results_visualization
singularity exec --bind $PWD:/data --pwd /data $MULTIQC_CONTAINER multiqc \
    /data/ANALYSIS/results_visualization \
